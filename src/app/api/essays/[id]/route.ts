...
https://localhost3000/api/essays/[id]

or

https://localhost3000/api/essays/682e36436d63526097a78fa1
...
export async function PUT(request: Request, { params }: { params: Promise<{id: string}> })
{
    ...
    const { newStudent_id: student_id, newCourse_id: course_id, newVaderData: vaderData } = await request.json();
    ...
    {
        // Update the db
        const updated = await Essay.findByIdAndUpdate(id, { student_id, course_id, vaderData });

        // Check to see if the essay was found
        if (!updated)
        {
            ...
            return NextResponse.json({ message: "Essay Updated" }, { status: 200 });
        }
    } catch (error) {
...
export async function DELETE(_request: Request, { params }: { params: Promise<{id: string}> })
{
    ...
    // Try to delete the Essay from the db using the id
    try 
    {
        // Delete the Essay from the db
        ...
        if (deleted)
        {
            ...
            return NextResponse.json({ message: "Essay Deleted" }, { status: 200 });
        } else {
...
export async function GET(_request: Request, { params }: { params: Promise<{id: string}> })
{
    ...
    // Try to get the Essay by it's id
    try
    {
        // Get the essay
